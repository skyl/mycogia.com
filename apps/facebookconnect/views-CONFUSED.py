...
def setup(request):
    if not request.facebook.uid:
        return HttpResponseRedirect(reverse('acct_login')+"?next="+request.GET.get('next',''))
    ...
    if request.method == "POST":
        ...
        profile.save()
        logging.info("FBC: Attached facebook profile %s to user %s!" % (profile.facebook_id,request.user))
